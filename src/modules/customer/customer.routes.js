 ...
 */
router.post('/purchase', requireCustomerAuth, async (req, res) => {
  try {
    const { requestCode, planType = 'ANNUAL_PRO' } = req.body;
    ...
    await bindMachine(clientId, finalRequestCode);
    const subResult = await createSubscription(clientId, finalRequestCode, planType);
