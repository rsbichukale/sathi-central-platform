...
router.post('/create-order', async (req, res) => {
  try {
    ...
    const amountPaise = await getAmountPaise(planType);
...
router.post('/verify-signature', async (req, res) => {
  try {
    ...
    if (razorpaySecret !== 'test_secret_key_2026') {
      if (!razorpay_signature) {
        return res.status(400).json({ success: false, error: 'Razorpay signature is missing.' });
      }
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    if (config.RAZORPAY_WEBHOOK_SECRET !== 'whsec_ruractive2026') {
      if (!signature) {
        return res.status(400).json({ success: false, error: 'Webhook signature is missing' });
      }
    ...
    if (event === 'payment.captured' || event === 'order.paid') {
      ...
      if (mobileNo) {
        ...
        // 1. Determine Plan Type based on Amount
        let planType = 'ANNUAL_PRO';
        if (amount >= 2999900) {
          planType = 'LIFETIME';
        } else if (amount >= 1299900) {
          planType = 'ENTERPRISE';
        }

        // 2. Find or Create Client
        const client = await findOrCreateClient({ mobileNo, email });
        ...
        // 4. Generate Key via createSubscription
        const subResult = await createSubscription(client.id, null, planType, { forceNew: true });
        
        logger.audit('PAYMENT', 'WEBHOOK_PAYMENT_CAPTURED', {
          clientId: client.id,
          details: { orderId, paymentId, amount, activationKey: subResult.activationKey }
        });

        // 5. Send Activation Email
        if (client.email || email) {
          sendActivationKeyEmail({
            toEmail: client.email || email,
            firmName: client.firm_name || 'Valued Customer',
            ownerName: client.owner_name || '',
            activationKey: subResult.activationKey,
            userApiKey: client.api_key,
            planType,
            expiresAt: subResult.expiresAt,
            requestCode: 'Auto-Paired (Bind on Login)'
          }).catch(err => logger.error('Email', 'Webhook activation email error', { error: err.message }));
        }
      }
    }
