...
const { validateMobile, validateEmail, validateGstin } = require('../middleware/auth');
...
router.post('/settings/update', async (req, res) => {
  try {
    const { mobileNo, firmName, ownerName, email, gstin, address, autoSyncFarmers, autoSyncDealers, syncIntervalMins } = req.body;
    if (!mobileNo) return res.status(400).json({ success: false, error: 'Mobile Number required.' });
